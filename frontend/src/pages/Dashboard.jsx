...
function Dashboard() {
  ...
  const fetchHabits = async () => {
    try {
      ...
      for (const habit of res.data) {
        try {
          const logsRes = await api.get(`/habits/${habit.id}/logs`);
          const todayLog = logsRes.data.find((l) => l.log_date.split('T')[0] === today);
          if (todayLog) logs[habit.id] = todayLog;
        } catch (e) {}
      }
      setTodayLogs(logs);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <h2 className="text-xl font-semibold mb-3">Your Habits — Today</h2>
      <div className="grid gap-3">
        {habits.map((habit) => {
          const log = todayLogs[habit.id];
          const completion = log ? Math.round(Number(log.completion_percent)) : 0;
          ...
          return (
            <div key={habit.id} className="bg-white p-4 rounded shadow">
              ...
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div
                  className={`h-3 rounded-full ${completion >= 100 ? 'bg-green-500' : 'bg-blue-500'}`}
                  style={{ width: `${completion}%` }}
                ></div>
              </div>
              <div className="text-right text-xs text-gray-500 mt-1">{completion}% completed today</div>
            </div>
          );
        })}
